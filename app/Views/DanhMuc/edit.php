...
<div class="container mx-auto px-4 py-8">
    ...
    <nav class="mb-8">
        <ol class="flex items-center space-x-2 text-sm text-slate-600">
            ...
            <li><span class="mx-2">/</span></li>
            <li class="text-slate-800 font-medium">Chỉnh sửa danh mục</li>
        </ol>
    </nav>

    <!-- Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-slate-800 mb-2">Chỉnh sửa danh mục</h1>
        <p class="text-slate-600">Cập nhật thông tin danh mục sản phẩm/dịch vụ</p>
    </div>
        ...
        <div class="xl:col-span-2">
            <form id="editCategoryForm" class="space-y-6">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" id="ten_danh_muc" name="ten_danh_muc" 
                                       value="<?= htmlspecialchars($danhMuc['ten_danh_muc'] ?? 'Danh mục phòng') ?>"
                            ...
                            <div>
                                ...
                                <input type="text" id="slug" name="slug" 
                                       value="<?= htmlspecialchars($danhMuc['slug'] ?? 'danh-muc-phong') ?>"
                                       class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all"
                                       required>
                                <p class="text-xs text-slate-500 mt-1">URL thân thiện cho danh mục</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select id="loai" name="loai" 
                                        class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all"
                                        required>
                                    <option value="phong" <?= ($danhMuc['loai'] ?? 'phong') == 'phong' ? 'selected' : '' ?>>Phòng</option>
                                    <option value="dich_vu" <?= ($danhMuc['loai'] ?? 'phong') == 'dich_vu' ? 'selected' : '' ?>>Dịch vụ</option>
                                    <option value="tien_ich" <?= ($danhMuc['loai'] ?? 'phong') == 'tien_ich' ? 'selected' : '' ?>>Tiện ích</option>
                                    <option value="am_thuc" <?= ($danhMuc['loai'] ?? 'phong') == 'am_thuc' ? 'selected' : '' ?>>Ẩm thực</option>
                                    <option value="su_kien" <?= ($danhMuc['loai'] ?? 'phong') == 'su_kien' ? 'selected' : '' ?>>Sự kiện</option>
                                    <option value="khac" <?= ($danhMuc['loai'] ?? 'phong') == 'khac' ? 'selected' : '' ?>>Khác</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <select id="danh_muc_cha_id" name="danh_muc_cha_id" 
                                    ...
                                    <option value="1" <?= ($danhMuc['danh_muc_cha_id'] ?? '') == '1' ? 'selected' : '' ?>>Phòng nghỉ</option>
                                    <option value="2" <?= ($danhMuc['danh_muc_cha_id'] ?? '') == '2' ? 'selected' : '' ?>>Dịch vụ spa</option>
                                    <option value="3" <?= ($danhMuc['danh_muc_cha_id'] ?? '') == '3' ? 'selected' : '' ?>>Nhà hàng</option>
                                    <option value="4" <?= ($danhMuc['danh_muc_cha_id'] ?? '') == '4' ? 'selected' : '' ?>>Tiện ích khách sạn</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select id="trang_thai" name="trang_thai" 
                                        class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all">
                                    <option value="1" <?= ($danhMuc['trang_thai'] ?? 1) == 1 ? 'selected' : '' ?>>Hoạt động</option>
                                    <option value="0" <?= ($danhMuc['trang_thai'] ?? 1) == 0 ? 'selected' : '' ?>>Vô hiệu hóa</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="mo_ta" name="mo_ta" rows="4" 
                                      ...
                                      placeholder="Nhập mô tả chi tiết về danh mục..."><?= htmlspecialchars($danhMuc['mo_ta'] ?? 'Danh mục chứa các phòng nghỉ cao cấp với đầy đủ tiện nghi hiện đại.') ?></textarea>
                    ...
                    <div class="p-6 space-y-4">
                        <div>
                            ...
                            <input type="text" id="meta_title" name="meta_title" 
                                   value="<?= htmlspecialchars($danhMuc['meta_title'] ?? 'Danh mục phòng - Khách sạn cao cấp') ?>"
                                   class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all"
                        ...
                        <div>
                            ...
                            <textarea id="meta_description" name="meta_description" rows="3" 
                                      class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent resize-none transition-all"
                                      placeholder="Mô tả ngắn gọn về danh mục cho SEO..."><?= htmlspecialchars($danhMuc['meta_description'] ?? 'Khám phá bộ sưu tập phòng nghỉ cao cấp với đầy đủ tiện nghi hiện đại và dịch vụ chăm sóc khách hàng tận tình.') ?></textarea>
                            <div class="flex justify-between items-center mt-1">
                        ...
                        <div>
                            ...
                            <input type="text" id="meta_keywords" name="meta_keywords" 
                                   value="<?= htmlspecialchars($danhMuc['meta_keywords'] ?? 'phòng nghỉ, khách sạn, booking, cao cấp') ?>"
                                   class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all"
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <?php if (isset($danhMuc['hinh_anh']) && $danhMuc['hinh_anh']): ?>
                                <div class="border-2 border-dashed border-slate-300 rounded-lg p-4 text-center">
                                    <img src="<?= htmlspecialchars($danhMuc['hinh_anh']) ?>" alt="Category Image" class="mx-auto h-24 w-auto rounded-lg mb-2">
                                    <p class="text-sm text-green-600 font-medium">Hình ảnh hiện tại</p>
                                    <button type="button" class="mt-2 text-xs text-cyan-600 hover:text-cyan-800" onclick="changeImage(this)">Thay đổi hình ảnh</button>
                                </div>
                                <?php else: ?>
                                <div class="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center hover:border-cyan-400 transition-colors" onclick="this.querySelector('input').click()">
                            ...
                            <div>
                                ...
                                <?php if (isset($danhMuc['banner']) && $danhMuc['banner']): ?>
                                <div class="border-2 border-dashed border-slate-300 rounded-lg p-4 text-center">
                                    <img src="<?= htmlspecialchars($danhMuc['banner']) ?>" alt="Category Banner" class="mx-auto h-24 w-auto rounded-lg mb-2">
                                    <p class="text-sm text-green-600 font-medium">Banner hiện tại</p>
                                    <button type="button" class="mt-2 text-xs text-cyan-600 hover:text-cyan-800" onclick="changeImage(this)">Thay đổi banner</button>
                                </div>
                                <?php else: ?>
                                <div class="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center hover:border-cyan-400 transition-colors" onclick="this.querySelector('input').click()">
                                    <svg class="mx-auto h-12 w-12 text-slate-400" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                ...
                <div class="flex flex-col sm:flex-row gap-4 justify-end">
                    <a href="/danh-muc/show/<?= $danhMuc['id'] ?? 1 ?>" 
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
                <div class="bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-4 border-b border-slate-200">
                    <h3 class="text-lg font-semibold text-slate-800">Thông tin danh mục</h3>
                </div>
                <div class="p-6 space-y-4">
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-slate-600">ID danh mục</span>
                        <span class="text-sm font-medium text-slate-800">#<?= $danhMuc['id'] ?? 1 ?></span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-slate-600">Ngày tạo</span>
                        <span class="text-sm text-slate-800"><?= date('d/m/Y', strtotime($danhMuc['ngay_tao'] ?? 'now')) ?></span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-slate-600">Cập nhật cuối</span>
                        <span class="text-sm text-slate-800"><?= date('d/m/Y', strtotime($danhMuc['ngay_cap_nhat'] ?? 'now')) ?></span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-slate-600">Lượt xem</span>
                        <span class="text-sm text-slate-800"><?= number_format($danhMuc['luot_xem'] ?? 1250) ?></span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-emerald-50 to-green-50 rounded-xl border border-emerald-200 p-6">
                <h3 class="text-lg font-semibold text-slate-800 mb-4">Thống kê sản phẩm</h3>
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-slate-600">Tổng sản phẩm</span>
                        <span class="text-lg font-bold text-emerald-600">25</span>
                    </div>
                    <div class="flex items-center justify-between">
            ...
            <div class="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
                ...
                <div class="p-6 space-y-3">
                    ...
                    <button class="w-full bg-red-500 hover:bg-red-600 text-white font-medium py-2 px-4 rounded-lg transition-colors text-sm">
                        Xóa danh mục
                    </button>
                </div>
            </div>
...
function changeImage(button) {
    ...
    container.innerHTML = `
        <div class="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center hover:border-cyan-400 transition-colors" onclick="this.querySelector('input').click()">
            ...
            <div class="mt-4">
                <p class="text-sm text-slate-600">Kéo thả hoặc click để chọn hình mới</p>
                <p class="text-xs text-slate-500">PNG, JPG, GIF tối đa 2MB</p>
            </div>
            <input type="file" class="hidden" accept="image/*">
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    // Auto-generate slug from category name (if manually changed)
    ...
    nameInput.addEventListener('input', function() {
        // Only auto-generate if user hasn't manually changed slug
        const newSlug = generateSlug(nameInput.value);
        if (slugInput.value === originalSlug || slugInput.value === generateSlug(originalSlug)) {
    ...
    actionButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (action.includes('Xóa') && !confirm(`Bạn có chắc chắn muốn ${action.toLowerCase()}?`)) {
                return;
            }
