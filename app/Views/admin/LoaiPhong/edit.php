            ...
            <form method="POST" action="/admin/loai-phong/update" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <select id="trang_thai" 
                            ...
                            required>
                        <?php
                        $trangThaiList = \HotelBooking\Enums\TrangThaiLoaiPhong::all();
                        $currentStatus = $loaiPhong->trang_thai ?? \HotelBooking\Enums\TrangThaiLoaiPhong::HOAT_DONG;
                        foreach ($trangThaiList as $status): ?>
                            <option value="<?= $status ?>" <?= $status === $currentStatus ? 'selected' : '' ?>>
                                <?= \HotelBooking\Enums\TrangThaiLoaiPhong::getLabel($status) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
